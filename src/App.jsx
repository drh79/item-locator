  ...
  const fetchNearbyStores = async (lat, lng) => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: { "Content-Type": "application/json", "x-api-key": import.meta.env.VITE_ANTHROPIC_API_KEY "anthropic-dangerous-direct-browser-access": "true"  },
        body: JSON.stringify({
        ...
        {step === "result" && result && (
          <div>
            ...
            <div style={{ background:"white", borderRadius:12, padding:16, boxShadow:"0 1px 4px rgba(0,0,0,0.08)", marginBottom:12 }}>
              ...
              {result.department && <div style={{ marginTop:10, background:"#fafafa", borderRadius:10, padding:"10px 12px", fontSize:13, color:"#475569" }}>🏬 Department: <strong>{result.department}</strong></div>}
            </div>
            ...
            {selectedStore.phone && (
              <button onClick={() => handleCall(selectedStore.phone)} style={{ width:"100%", padding:"13px", background:"#f0fdf4", color:"#16a34a", border:"1.5px solid #bbf7d0", borderRadius:12, fontSize:15, fontWeight:600, cursor:"pointer", marginBottom:8 }}>
                📞 Call {selectedStore.name} to Confirm
              </button>
            )}
            <button onClick={() => { setStep("searching"); setResult(null); setSearchItem(""); }} style={{ width:"100%", padding:"13px", background:"#1e40af", color:"white", border:"none", borderRadius:12, fontSize:15, fontWeight:600, cursor:"pointer", marginBottom:8 }}>
              🔍 Search Another Item
            </button>
            <button onClick={() => { setStep("stores"); setSelectedStore(null); setResult(null); setSearchItem(""); }} style={{ width:"100%", padding:"13px", background:"white", color:"#1e40af", border:"1.5px solid #1e40af", borderRadius:12, fontSize:15, fontWeight:600, cursor:"pointer" }}>
              🏪 Change Store
            </button>
          </div>
        )}
